...
void UI::update(int rotationCount) {
  if ((refreshCount - lastRefresh) > 500)
  {
    ...
    // Performing this update takes around 10 ms with both fastlib also with arduino built-in 
    // LiquidCrystal library - Mileage may vary depending on the LCD display controller
    //
    // Uncomment the surrounding profiling pin statements, make sure they are not toggled anywhere
    // else and masure to check for your specific LCD
